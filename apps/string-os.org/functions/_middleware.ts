/**
 * Cloudflare Pages Function — Accept-based content negotiation for string-os.org.
 *
 * Ported from the agentnews CF middleware. Replaces the Vercel `middleware.ts`
 * when this site runs on Cloudflare Pages:
 *   - Browsers (Accept: text/html) get Astro's static HTML.
 *   - Agents (Accept: text/markdown) get the parallel `.md` twin from the same URL.
 *   - Direct `*.md` requests are served as markdown.
 * Every markdown read is logged to a Cloudflare Analytics Engine dataset
 * (`STRING_ANALYTICS` binding), classifying agent vs human by User-Agent — the
 * edge-level signal a client-side JS beacon cannot capture. Binding is optional:
 * without it, negotiation still works and logging is skipped.
 */
type Env = {
  STRING_ANALYTICS?: AnalyticsEngineDataset;
};
...
function logMarkdownRead(context: EventContext<Env, string, unknown>, pathname: string, userAgent: string) {
  const dataset = context.env.STRING_ANALYTICS;
  if (!dataset) return;
