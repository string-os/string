/**
 * Vercel Routing Middleware — SFMD content negotiation for string-os.org.
 *
 ...
 * any static host. Hosts without request-time logic (e.g. GitHub Pages) just
 * let agents fetch the `.md` URL directly — no middleware needed.
 *
...
export const config = {
  // Page routes only. The negative lookahead skips Astro's hashed assets and
  // any path that already has a file extension — which covers /favicon.* and
  // direct /*.md requests, so we never rewrite a .md path to .md.md.
  matcher: ['/((?!_astro/|.*\\.).*)'],
};
